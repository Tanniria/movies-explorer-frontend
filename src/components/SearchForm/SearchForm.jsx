...
export default function SearchForm({data, setMovies, url}) {
	...
	const filterAndSearch = () => {
		...
		if (obj.isChecked && !obj.query) {
			...
			result = data.movies.filter((m) => m.nameRU.toLowerCase().includes(obj.query.toLowerCase()) || m.nameEN.toLowerCase().includes(obj.query.toLowerCase()));
			localStorage.setItem(
				url,
				JSON.stringify({
					...obj,
			...
			result = data.movies.filter((m) => m.nameRU.toLowerCase().includes(obj.query.toLowerCase()) || m.nameEN.toLowerCase().includes(obj.query.toLowerCase())).filter((m) => m.duration < 40);
			localStorage.setItem(
				url,
				JSON.stringify({
					...obj,
	...
	return (
		<section className='search-form'>
			<form
				...
				onSubmit={(e) => handleChange(e, 'submit')}>
