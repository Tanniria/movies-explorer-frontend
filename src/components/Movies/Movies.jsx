...
export default function Movies() {
  ...
  useEffect(() => {
    const getMovies = async () => {
      try {
        const [allMoviesData, savedMoviesData] = await Promise.all([
          getAllMovies(),
          getSavedMovies(),
        ]);
        setMoviesState((prev) => ({
  ...
  const handleRemoveMovie = (movie) => {
    const removedMovie = savedMovies.movies.find(
  ...
  return (
    <>
      ...
      <main className="movies">
        <SearchForm data={movies} setMovies={setMovies} url={url} />
        {isLoaded ? (
          <MoviesCardList
            movies={
              filterMode
                ? movies.searchedMovies.slice(0, moviesList.quantityMovies)
                : movies.movies.slice(0, moviesList.quantityMovies)
            }
        ...
        {moviesList.quantityMovies <= movies.movies.length && (
          <button className="movies-button" type="button" onClick={handleAddBtn}>
            Ещё
          </button>
        )}
      </main>
